...
2. **Node.js-Version für Check-and-Lint aktualisiert**: Die im `check-and-lint`-Job verwendete Node.js-Version wurde auf `__DEFAULT_NODEJS__` gesetzt.

3. **Node.js-Version für Deployment aktualisiert** (falls zutreffend): Die im `deploy`-Job verwendete Node.js-Version wurde auf `__DEFAULT_NODEJS__` gesetzt. Diese Änderung wird nur angewendet, wenn der Deploy-Schritt vorhanden und nicht auskommentiert ist.
