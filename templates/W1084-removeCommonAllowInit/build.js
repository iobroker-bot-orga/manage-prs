...
// Check if this is a scheduled adapter - if so, allowInit is valid and should not be removed
if (ioPackage.common?.mode === 'schedule') {
    console.log(`✔️ Adapter is of type 'schedule', common.allowInit is valid for scheduled adapters. No PR needed.`);
    process.exit(0);
}

// Check if common.allowInit exists
if (ioPackage.common?.allowInit === undefined) {
    console.log(`✔️ common.allowInit does not exist, no need for a PR.`);
    process.exit(0);
}

console.log(`ⓘ common.allowInit exists with value: ${ioPackage.common.allowInit}, proceeding with removal.`);
console.log(`ⓘ This adapter is not of type 'schedule'. The allowInit attribute is ignored for non-scheduled adapters.`);

// Use string-based removal to preserve original formatting
// This approach ensures we only remove the "allowInit" line without changing indentation or property order
...
// Find the line with "allowInit" in the common section only
// Pattern explanation:
// - (\r?\n)? : Group 1 - Optional preceding newline (this belongs to previous line, we preserve it)
// - (?:\s*) : Non-capturing - Indentation whitespace at start of allowInit line
// - "allowInit"\s*:\s*(?:true|false) : The property key-value (boolean)
...
const allowInitLinePattern = /(\r?\n)?(?:\s*)"allowInit"\s*:\s*(?:true|false)(?:[ \t]*)(,?)(?:[ \t]*)(?:\r?\n)?/;

const match = commonContent.match(allowInitLinePattern);

if (!match) {
    console.error(`❌ Could not find "allowInit" line in common section of ${ioPackagePath}`);
    process.exit(1);
}
...
function removePropertyLine(content, lineStart, lineEnd, hasTrailingComma) {
    ...
    if (searchPos >= 0 && content[searchPos] === ',') {
        // Remove the comma from the previous line and the entire allowInit line
...
console.log(`✔️ Removed common.allowInit from ${ioPackagePath}`);
console.log(`ⓘ common.allowInit has been removed. This adapter is not of type 'schedule', and allowInit is ignored for non-scheduled adapters anyway.`);
