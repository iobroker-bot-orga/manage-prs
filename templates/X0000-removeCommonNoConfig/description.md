Remove deprecated common.noConfig from io-package.json
...
This PR updates `io-package.json` because `common.noConfig` is deprecated since `js-controller` 5 and has been replaced by `common.adminUI` configuration.

The `common.noConfig` attribute has been removed.
...
Dieser PR aktualisiert die `io-package.json`, weil `common.noConfig` seit `js-controller` 5 veraltet ist und durch die `common.adminUI`-Konfiguration ersetzt wurde.

Das Attribut `common.noConfig` wurde entfernt.
