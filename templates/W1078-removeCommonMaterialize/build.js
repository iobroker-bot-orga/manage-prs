// This template script updates io-package metadata for removing common.materialize.
// Required CLI inputs: templateName, repositoryName, optional parameterData.
// Behavior:
// - Exit without file changes when common.materialize does not exist.
// - Remove common.materialize when present.
// - For materialize !== false, align common.adminUI.config and js-controller dependency.
...
if (!ioPackage.common || typeof ioPackage.common !== 'object' || Array.isArray(ioPackage.common)) {
  console.log('ⓘ common does not exist, common.materialize cannot exist. No PR required.');
  console.log('✔️ processing completed');
  process.exit(0);
}

if (!Object.prototype.hasOwnProperty.call(ioPackage.common, 'materialize')) {
  console.log('ⓘ common.materialize does not exist. No PR required.');
  console.log('✔️ processing completed');
  process.exit(0);
}

const materializeValue = ioPackage.common.materialize;
delete ioPackage.common.materialize;
console.log('✔️ Removed common.materialize from io-package.json.');
...
if (materializeValue === false) {
  adminUiEn = 'The `common.adminUI.config` setting has not been changed because `common.materialize` was set to `false`.';
  dependencyEn = 'The `js-controller` dependency has not been changed for this PR because `common.materialize` was set to `false`.';
  adminUiDe = 'Die Einstellung `common.adminUI.config` wurde nicht geändert, weil `common.materialize` auf `false` gesetzt war.';
  dependencyDe = 'Die `js-controller`-Abhängigkeit wurde in diesem PR nicht geändert, weil `common.materialize` auf `false` gesetzt war.';
  console.log('ⓘ common.materialize was false, skipping adminUI and dependency changes.');
} else {
  if (!ioPackage.common.adminUI || typeof ioPackage.common.adminUI !== 'object' || Array.isArray(ioPackage.common.adminUI)) {
  ...
  if (Object.prototype.hasOwnProperty.call(ioPackage.common.adminUI, 'config')) {
    ...
    ioPackage.common.adminUI.config = 'materialize';
    adminUiEn = 'The `common.adminUI.config` setting did not exist and has been added with value `materialize`.';
    adminUiDe = 'Die Einstellung `common.adminUI.config` war nicht vorhanden und wurde mit dem Wert `materialize` ergänzt.';
    console.log('✔️ Added common.adminUI.config with value "materialize".');
  }
