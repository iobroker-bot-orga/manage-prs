...
if (!workflowContent.includes("ioBroker/testing-action-deploy@v1")) {
  console.log(
    `ⓘ Workflow does not use action 'ioBroker/testing-action-deploy@v1', no changes needed.`,
  );
  process.exit(0);
}
...
if (!workflowContent.includes("npm-token:")) {
  console.log(
    `ⓘ Action 'ioBroker/testing-action-deploy@v1' does not use parameter 'npm-token', no changes needed.`,
  );
  process.exit(0);
}
...
const npmTokenLines = workflowContent
  .split("\n")
...
if (allCommented && npmTokenLines.length > 0) {
  console.log(
    `ⓘ Parameter 'npm-token' is already commented out, no changes needed.`,
  );
  process.exit(0);
}
  ...
  if (lines[i].includes("ioBroker/testing-action-deploy@v1")) {
    ...
    for (let j = i + 1; j < lines.length; j++) {
      ...
      if (
        inWithBlock &&
        line.match(/^\s+\w+:/) &&
        currentLineIndent <= usesLineIndent
      ) {
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes("npm-token:") && i > deployActionLineIndex) {
    ...
    lines[i] =
      `${indent}# ${restOfLine}  # Commented out for migration to Trusted Publishing`;
...
if (stepStartIndex === -1) {
    ...
    for (let i = stepStartIndex - 1; i >= 0; i--) {
      ...
      const isJobKey =
        ...
        !line
          .trim()
          .match(
            /^(steps|with|env|outputs|strategy|matrix|runs-on|name|needs|if|uses|run|shell|continue-on-error|timeout-minutes|environment|permissions|secrets|container|services|defaults):\s*$/,
          );
      if (isJobKey) {
        jobLineIndex = i;
  ...
  for (let i = jobLineIndex + 1; i < deployActionLineIndex; i++) {
    ...
    const match = line.match(
      /^(\s+)(needs|runs-on|if|steps|strategy|environment|timeout-minutes|continue-on-error|container|services):/,
    );
    if (match) {
      jobPropertyIndent = match[1].length;
      console.log(
        `✔️ Detected job property indentation: ${jobPropertyIndent} spaces.`,
      );
      break;
    }
  }
  ...
  if (hasPermissions) {
    console.log(
      `✔️ Found existing permissions block at line ${permissionsLineIndex + 1}.`,
    );
    ...
    lines.splice(insertIndex, 0, ...newPermissions);
    console.log(
      `✔️ Added permissions block with 'contents: write' and 'id-token: write' at job level.`,
    );
    modified = true;
  }
}
